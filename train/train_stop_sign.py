...
parser.add_argument('--numberOfTrainingSamples', help="The number of training samples generated per epoch. Default: 5000", type=int, default=5000)
...
class FixedImageHeatmaps(Dataset):  # For validation, we'll use a fixed dataset
    def __init__(self, image_heatmap_filepath_pairs, maximum_number_of_samples=None):
        super().__init__()
...
class DynamicImageHeatmaps(Dataset):  # For training, we'll generate the data on the fly
    def __init__(self, image_sizeHW, number_of_samples):
        super().__init__()
...
def main():
    ...
    neural_network = None
    if args.architecture == 'FConv_3k5_16_32_64':
        neural_network = architectures.FConv_3k5(number_of_channels=(16, 32, 64), dropout_ratio=args.dropoutRatio)
    elif args.architecture == 'FConv_3k5_32_64_128':
        neural_network = architectures.FConv_3k5(number_of_channels=(32, 64, 128), dropout_ratio=args.dropoutRatio)
    elif args.architecture == 'FConv_4k5_16_32_64_128':
        neural_network = architectures.FConv_4k5(number_of_channels=(16, 32, 64, 128), dropout_ratio=args.dropoutRatio)
    elif args.architecture == 'FConv_4k5_32_64_128_256':
        neural_network = architectures.FConv_4k5(number_of_channels=(32, 64, 128, 256), dropout_ratio=args.dropoutRatio)
    elif args.architecture == 'FConv_4k5_64_128_256_512':
        neural_network = architectures.FConv_4k5(number_of_channels=(64, 128, 256, 512), dropout_ratio=args.dropoutRatio)
    elif args.architecture == 'FConv_5k5_16_32_64_128_256':
        neural_network = architectures.FConv_5k5(number_of_channels=(16, 32, 64, 128, 256), dropout_ratio=args.dropoutRatio)
    else:
        raise NotImplementedError("train_stop_sign.py main(): Not implemented architecture '{}'".format(args.architecture))
    ...
    with open(os.path.join(args.outputDirectory, 'epochLoss.csv'), 'w+', buffering=1) as epoch_loss_file:
        ...
        for epoch in range(1, args.numberOfEpochs + 1):
            ...
            for (input_tsr, target_heatmap_tsr) in training_loader:
            ...
            with torch.no_grad():
                ...
                number_of_minibatches = 0
                for (val_input_tsr, val_target_heatmap_tsr) in validation_loader:
                    val_input_tsr = val_input_tsr.to(device)
                    val_target_heatmap_tsr = val_target_heatmap_tsr.to(device)
                    val_output_tensor = neural_network(val_input_tsr)
